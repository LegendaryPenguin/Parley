...
Built for the **0G Zero Cup**, Parley combines verifiable TEE inference, user-owned encrypted storage, zero-trust client-side encryption, and on-chain anchoring into one product where the AI isn't a feature bolted on — it *is* the game. Remove 0G and you have an empty map with nothing to say, nothing remembered, and nothing to prove.
...
Parley turns "can you actually say it?" into a game loop — playable, private, and provable.
...
Parley is built as three connected layers, joined by a single seam, so the world and the 0G integration could be built in parallel.
...
- **Credentials:** soulbound INFT stage credential (`contracts/ParleyStageCredential.sol`, Foundry), token data bound to the 0G Storage transcript
...
**Nischay Rawal — full-stack, 0G integration & design**

I learned how to make decentralized AI carry an entire product instead of sitting next to it. The biggest unlock was **designing a seam**: putting every 0G call behind one typed `lib/og` interface — with mock and live implementations that toggle independently per service — meant the whole game was playable offline on day one, and swapping in real testnet Compute, Storage, and Chain never touched a single screen.

Reasoning about **what belongs on-chain versus off** completely reshaped the record design — raw transcripts and the model's reasoning stay encrypted in Storage or sealed in the TEE, and only the verdict's attestation and a hash anchor ever go on-chain. I also learned that **live inference is a UX material**: real testnet latency (~3–6s/turn) changes how a conversation should *feel*, and the "thinking…" beat, optimistic UI, and the two-call turn/judge split all came out of designing around real inference instead of a mock. Finally, decentralized AI is what made the pitch **honest** — "an AI graded you and you can prove it" is only true because the grade came from a TEE-attested model whose fingerprint is anchored on-chain.

## 🚀 Setup

Parley runs entirely offline by default (mock mode) and turns into a live 0G app one env flag at a time.

**Prerequisites**

- Node.js 18+ and npm
- (For live inference) a 0G testnet Router API key from [pc.testnet.0g.ai](https://pc.testnet.0g.ai), funded via [faucet.0g.ai](https://faucet.0g.ai)
- (For opt-in Storage / Chain) MetaMask on **0G Galileo testnet** (chainId 16602)

**1. Clone the repository**

```bash
git clone https://github.com/LegendaryPenguin/Parley.git
cd Parley
```

**2. Install and run (mock mode — no wallet, no network)**
...
With the default `OG_MODE=mock`, the whole golden path (Arrival → Atlas → Scene → reward → Passport) plays with canned dialogue in all seven languages + local storage.
...
**4. INFT stage credential (optional)**

The soulbound credential contract lives in `contracts/` (Foundry). See [contracts/README.md](contracts/README.md) to build, test, and deploy it.
